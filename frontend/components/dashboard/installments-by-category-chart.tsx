/**
 * Installments by Category Chart Widget
 * Shows installment breakdown by category using pie/donut chart
 */
'use client';
...
interface InstallmentsByCategoryChartProps {
  data: CategoryData[];
  isLoading?: boolean;
  chartType?: 'pie' | 'donut';
  currency?: string;
...
export function InstallmentsByCategoryChart({
  data,
  isLoading = false,
  chartType = 'donut',
  currency = 'USD',
}: InstallmentsByCategoryChartProps) {
    ...
    return (
      <Card className="p-4 md:p-6">
        <div className="flex flex-col items-center justify-center h-[300px] md:h-[400px] text-gray-500">
          ...
          <p className="text-sm md:text-base">No installment data available</p>
        </div>
      </Card>
    );
  }
      ...
      <Card className="p-4 md:p-6">
        <div className="mb-4 md:mb-6">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-base md:text-lg font-semibold">Installments by Category</h3>
            <ChartTooltip>
              <TooltipTrigger asChild>
                <Info className="h-4 w-4 text-gray-400 cursor-help" />
              </TooltipTrigger>
              <TooltipContent>
                <p className="max-w-xs">Shows all active installments grouped by category. Amounts are monthly equivalents regardless of payment frequency.</p>
              </TooltipContent>
            </ChartTooltip>
          </div>
          <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">
            Monthly installment payments by category
          </p>
        </div>
