...
- **AI**: LangGraph agent + pgvector RAG over your data (OpenAI `gpt-4o-mini` · `text-embedding-3-small`)
...
Wealth Vault ships an **AI financial assistant** — "Ask your finances" (`/dashboard/ask`). Ask
questions in plain English and get answers grounded in *your* data, with exact figures and the
source rows cited. It's built as an explicit **LangGraph `StateGraph`**, not a single prompt:

- **Hybrid retrieval** — numeric questions run 25 typed, user-scoped compute tools that return
  exact aggregates (no text-to-SQL); qualitative questions use **pgvector** RAG. The numbers
  never come from the LLM.
- **Deterministic validation** — a no-LLM node verifies every figure in the answer is derivable
  from the tools; an ungrounded number triggers a re-synthesis, then a refusal.
- **Streaming** — Server-Sent Events surface graph steps (`status`) and answer tokens for a fast
  time-to-first-token (`POST /api/v1/agent/query` · `/stream`).
- **Propose & confirm writes** — for "add a $20 expense" the agent drafts a typed action the user
  explicitly confirms (`POST /api/v1/agent/actions/confirm`); the graph never mutates data itself.
- **Tested in CI** — Promptfoo evals hit the real endpoint (exact number / refusal / citation +
  groundedness) with a cost/latency budget, plus per-action test coverage and secret/dependency scans.

The diagram below is generated directly from the compiled `StateGraph`
...
📄 Full design write-up, tradeoffs, and local setup: **[backend/AI_FEATURES.md](./backend/AI_FEATURES.md)**.
...
# Regenerate the agent graph diagram (keeps README + AI_FEATURES.md in sync)
DATABASE_URL=postgresql://x SECRET_KEY=x python -m app.scripts.draw_graph --write

# Open the AI agent in LangGraph Studio (pip install -r requirements-dev.txt first)
langgraph dev
```
...
- **LangGraph** - AI agent orchestration (`StateGraph`: router + validation cycle)
- **pgvector** - Vector similarity search inside Postgres (HNSW, cosine)
- **OpenAI** - `gpt-4o-mini` (agent) + `text-embedding-3-small` (embeddings) via `langchain-openai`
